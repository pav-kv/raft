...
// needStorageAppendRespMsg returns true if raft needs to hear about stabilized
// entries or an applied snapshot. See the comment in appendTracker.
func needStorageAppendRespMsg(rd Ready) bool {
	return len(rd.Entries) > 0 || !IsEmptySnap(rd.Snapshot)
}
...
func newStorageAppendRespMsg(r *raft, rd Ready) pb.Message {
	...
	if ln := len(rd.Entries); ln > 0 {
		// TODO(during review): simplify the comment, and place next to appendTracker.
		if err := r.appTracker.add(appendMark{term: r.Term, index: rd.Entries[0].Index}); err != nil {
			r.logger.Panicf("async appends tracking error: %v", err)
		}
		// The response triggers r.appTracker.ack() with this (m.Term, m.Index) mark.
		m.Index = rd.Entries[ln-1].Index
		m.LogTerm = rd.Entries[ln-1].Term
	}
	if !IsEmptySnap(rd.Snapshot) {
		snap := rd.Snapshot
		m.Snapshot = &snap
